...
DESCRIPTION="Efficient micro-compositor for running games (ChimeraOS's fork)"
...
if [[ ${PV} == "9999" ]]; then
	...
	SRC_URI="
		https://github.com/OpenGamingCollective/${PN}/archive/refs/tags/${MY_PV}.tar.gz -> ${P}.tar.gz
...
FILECAPS=(
	cap_sys_nice usr/bin/gamescope
)
...
src_prepare() {
	...
	# Unbundle glm and stb since its already packaged
	sed -i  's|stb_image_resize.h|deprecated/stb_image_resize.h|g' src/steamcompmgr.cpp || die
	sed -i  's|stb_image_resize.h|deprecated/stb_image_resize.h|g' src/reshade_effect_manager.cpp || die
	
	#sed -i "/glm_dep = glm_proj.get_variable('glm_dep')/d" meson.build || die
        #sed -i "/stb_dep = stb_proj.get_variable('stb_dep')/d" meson.build || die
        #sed -i "s|glm_proj = subproject('glm')|glm_dep = include_directories('/usr/include/glm')|g" meson.build || die
	#sed -i "s|stb_proj = subproject('stb')|stb_dep = include_directories('/usr/include/stb')|g" meson.build || die
	#sed -i "/glm_dep, /d" layer/meson.build || die
	#sed -i "/glm_dep, /d" src/meson.build || die
	#sed -i "/stb_dep, /d" src/meson.build || die
	default
}
